...
class SuperPointMatchingMutualTopk_myself(SuperPointMatchingMutualTopk):
    def __init__(self, num_correspondences, k, threshold=None, mutual=True, eps=1e-8):
        ...
        self.mutual = mutual
        self.eps = eps
    def forward(self, src_feats, tgt_feats1 ,tgt_feats2,tgt_feats3,tgt_feats4, src_masks=None, tgt_masks=None):
        ...
        if tgt_masks is not None:
            ...
            tgt_feats4 = tgt_feats4[tgt_masks]
        ...
        score_mat4 = torch.sqrt(pairwise_distance(src_feats, tgt_feats4, normalized=True) + self.eps)
        score_mat5  = torch.cat([score_mat1.unsqueeze(0), score_mat2.unsqueeze(0), score_mat3.unsqueeze(0),score_mat4.unsqueeze(0)], dim=0)
        score_mat, score_mat_indices = torch.max(score_mat5,dim=0)
        
        # 第一种dual_softmax
        # scoremat_column = F.softmax(score_mat5, dim=0)
        # scoremat_row =  F.softmax(score_mat5, dim=1)
        # score_mat = scoremat_column*scoremat_row

        #第二种dual_softmax
        # scoremat_row = F.softmax(score_mat5, dim=1)
        # score_mat6 = score_mat5*scoremat_row
        # score_mat = F.softmax(score_mat6, dim=0)
...
class GRL(nn.Module):
    ...
    def forward(self, input: torch.Tensor, coeff: float = 0.003):
...
class B23D(nn.Module):
    def __init__(self, cfg):
        ...
        # self.p_linear_layer = None
        # nn.init.normal_(self.p_linear_layer.weight, mean=0, std=0.01)
        # nn.init.constant_(self.p_linear_layer.bias, 0)

        self.p_layer = nn.Conv1d(256,256,kernel_size=1)
    ...
    def forward(self, data_dict):
        ...
        # i0_sig2s = i0_sig1.squeeze()#[256]
        # i0_sig2 = torch.diag_embed(i0_sig2s**2)

        # d0 = i0_sig2.size(0)  # 获取对角矩阵的维度
        # log_det_sigma0 = torch.sum(torch.log(torch.diag(i0_sig2)))  # 计算对角协方差矩阵的行列式的对数
        ...
        # all_img_feats_c_0 = i0_mu3 + all_img_feats_c_0_sample*i0_sig2s
        ...
        # i1_sig2s = i1_sig1.squeeze()#[256]
        # i1_sig2 = torch.diag_embed(i1_sig2s**2)

        # d1 = i1_sig2.size(0)  # 获取对角矩阵的维度
        # log_det_sigma1 = torch.sum(torch.log(torch.diag(i1_sig2)))  # 计算对角协方差矩阵的行列式的对数
        ...
        # all_img_feats_c_1 = i1_mu3 + all_img_feats_c_1_sample*i1_sig2s
        ...
        i0_mu1 = torch.mean(all_img_feats_c0, dim=[1, 2], keepdim=True)#做均值 #变为[1,256,24,32]
        i0_sig = self.i0_conv(all_img_feats_c0)#[1,256,1,1]
        i0_sig_r = F.relu(i0_sig)
        i0_sig1 = i0_sig_r.squeeze(0)#[256,1,1]
        i0_mu2 = F.dropout(i0_mu1, p=0.5, training=True)
        i0_mu3 = i0_mu2.squeeze()#[256]
        i0_sig2 = i0_sig1.squeeze()#[256]
        # i0_sig2s = F.softplus(i0_sig2)
        i0_sig2 = i0_sig2 + 1e-6
        ...
        i1_mu1 = torch.mean(all_img_feats_c1, dim=[1, 2], keepdim=True)#做均值 #变为[1,256,12,16]
        i1_sig = self.i1_conv(all_img_feats_c1)#[1,256,1,1]
        i1_sig_r = F.relu(i1_sig)
        i1_sig1 = i1_sig_r.squeeze(0)#[256,1,1]
        i1_mu2 = F.dropout(i1_mu1, p=0.5, training=True)
        i1_mu3 = i1_mu2.squeeze()#[256]
        i1_sig2 = i1_sig1.squeeze()#[256]
        # i1_sig2s = F.softplus(i1_sig2)
        i1_sig2 = i1_sig2 + 1e-6
        ...
        i4,q4 = self.crosstransformer(
            all_img_feats_c[2].unsqueeze(0),
            img_pixels_c.unsqueeze(0),
            q3,
            pcd_points_c.unsqueeze(0)       
            )
        ...
        if not self.training:
            img_node_corr_indices, pcd_node_corr_indices, node_corr_scores = self.coarse_matching(
                img_feats_c, q1,q2,q3,q4, img_node_masks, pcd_node_masks)
