...
class CoarseMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict):
        ...
        q2 = output_dict["q2"]
        q3 = output_dict["q3"]
        entropy0 = output_dict["entropy0"]
        entropy1 = output_dict["entropy1"]
        img_node_masks = output_dict["img_node_masks"]
        pcd_node_masks = output_dict["pcd_node_masks"]
        ...
        gt_node_corr_max_overlaps = output_dict["gt_node_corr_max_overlaps"]
        
    
        
        score_mat1 = torch.sqrt(pairwise_distance(img_feats, q1, normalized=True) + self.eps)
        score_mat2 = torch.sqrt(pairwise_distance(img_feats, q2, normalized=True) + self.eps)
        score_mat3 = torch.sqrt(pairwise_distance(img_feats, q3, normalized=True) + self.eps)
        score_mat4  = torch.cat([score_mat1.unsqueeze(0), score_mat2.unsqueeze(0), score_mat3.unsqueeze(0)], dim=0)
        score_mat5, score_mat_indices = torch.max(score_mat4,dim=0)

        # feat_dists1 = torch.sqrt(pairwise_distance(img_feats, q1, normalized=True) + self.eps)
        min_overlaps = torch.zeros_like(score_mat5)
        ...
        max_overlaps = torch.zeros_like(score_mat5)
        ...
        loss_circle = self.weighted_circle_loss(pos_masks, neg_masks, score_mat5, pos_scales)

        lfu_loss0 = self.lfu_loss(entropy0, gamma=15)
        lfu_loss1 = self.lfu_loss(entropy1, gamma=15)

        loss = loss_circle + lfu_loss0 + lfu_loss1

        return loss

    def lfu_loss(self, entropy, gamma):
        """
        Compute the Lfu loss.

        Parameters:
        sig2s (torch.Tensor): Tensor of shape (256,) containing the variance values (sigma^2).
        gamma (float): Threshold value γ.

        Returns:
        torch.Tensor: The computed Lfu loss.
        """
        sum_entropy = torch.sum(entropy)  # Sum of all variance values
        loss = torch.clamp(gamma - sum_entropy, min=0)  # Compute max(0, γ - sum(q))
        return loss
...
class ClassificationLoss(nn.Module):
    ...
    def forward(self, output_dict):
        class_preds = output_dict["class_preds"]
        all_labels = output_dict["all_labels"]
        classification_loss = self.cross_entropy_loss(class_preds, all_labels)
